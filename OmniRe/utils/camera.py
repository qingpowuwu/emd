...
def get_interp_novel_trajectories(
    ...
    trajectory_generators = {
        ...
        "change_lane_left_2m": change_lane_left_2m,
        "change_lane_left_2.5m": change_lane_left_2_5m,
        "change_lane_right_2m": change_lane_right_2m,
        "change_lane_right_2.5m": change_lane_right_2_5m
    }
...
def center_interp(
    ...
    """Interpolate key frames from the front center camera."""
    assert 0 in per_cam_poses.keys(), "Front center camera (ID 0) is required for front_center_interp"
    key_poses = per_cam_poses[0]  # Select every 4th frame as key frame
    return interpolate_poses(key_poses, target_frames)

def change_lane_left_2_5m(
    dataset_type: str, per_cam_poses: Dict[int, torch.Tensor], original_frames: int, target_frames: int, num_loops: int = 1
) -> torch.Tensor:
    """左变道,2.5米(平滑线性横移)"""
    cam_poses = per_cam_poses[0]
    if target_frames > len(cam_poses):
        cam_poses = center_interp(dataset_type, per_cam_poses, original_frames, target_frames)
    cam_poses = cam_poses.clone()
    N = len(cam_poses)
    for i in range(N):
        cam_poses[i, 1, 3] += 2.5 * i / (N - 1)
    return cam_poses

def change_lane_left_2m(
    dataset_type: str, per_cam_poses: Dict[int, torch.Tensor], original_frames: int, target_frames: int, num_loops: int = 1
) -> torch.Tensor:
    cam_poses = per_cam_poses[0]
    if target_frames > len(cam_poses):
    ...
    N = len(cam_poses)
    for i in range(N):
        cam_poses[i, 1, 3] += 2.0 * i / (N - 1)
    return cam_poses


def change_lane_right_2m(
    dataset_type: str, per_cam_poses: Dict[int, torch.Tensor], original_frames: int, target_frames: int, num_loops: int = 1
) -> torch.Tensor:
    cam_poses = per_cam_poses[0]
    if target_frames > len(cam_poses):
