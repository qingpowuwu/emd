...
current_time = time.strftime("%Y_%m-%d_%H-%M_%S", time.localtime())
...
def setup(args):
    ...
    log_dir = log_dir + f"-{current_time}"
...
def main(args):
    ...
    trainer = import_str(cfg.trainer.type)(
        **cfg.trainer,
        ...
        num_train_images=len(dataset.train_image_set), # 150
        num_full_images=len(dataset.full_image_set), # 150
    ...
    do_evaluation(
        step=float(step),  # 确保是 Python 原生类型
        cfg=cfg,
        trainer=trainer,
        dataset=dataset,
        render_keys=render_keys,
